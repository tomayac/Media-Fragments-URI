...
// @version       0.1.0
// @description   Adds Media Fragments URIs support to pages with HTML5 video.
// @include       http://*
// ==/UserScript==
...
(function(d) {
  ...
  script.src = 'https://raw.github.com/tomayac/Media-Fragments-URI/master/mediafragments.js';
  script.type = 'application/javascript';
  script.id = MEDIAFRAGMENTS;
  script.addEventListener('load', function() {
  */

  // listen on dynamic video insertion
  ...
  */
  ...
  function checkVideos(videos) {
    ...
    for (var i = 0, video; video = videos[i]; i++) {    
      ...
      video.classList.add(MEDIAFRAGMENTS);
      var src = video.currentSrc;
      if (DEBUG) console.log('Found a <video> with @src="' + src + '".');
      var frag = MediaFragments.parse(src);
      var hash = frag.hash;
      if (hash.t && Array.isArray(hash.t)) {
        if (DEBUG) console.log('The <video> has a media fragments URI.');
        var time = hash.t[0];
        if (time.unit === 'npt') {
          var start = time.startNormalized;
          if (start >= 0) {
